...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
    {
        ...
        var limit = Convert.ToInt32(request.QueryStringParameters["limit"] ?? "10");

        var startKey = request.QueryStringParameters["startKey"];

        List<Chat> chats = await GetAllChats(userId, limit, startKey);
    ...
    private async Task<List<Chat>> GetAllChats(string userId)
    {
        ...
        var user2 = new QueryOperationConfig()
        {
            ...
            Limit = limit - user1Results.Count
        };
